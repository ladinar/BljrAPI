...
public class DashboardActivity extends AppCompatActivity {
    ...
    List<SliceValue> donutData = new ArrayList<>();
    //    List<Integer> Bulan = new ArrayList<>();
    List<String> status = new ArrayList<>();
    List<Integer> warna_status = new ArrayList<>();
    BarChart barChart;
    PieChartView pieChartView, doughnutChartView;
            ...
            public void onResponse(JSONObject response) {
                try {
                    ...
                    if (response.length() > 0) {
                        ...
                        //kok iso syad?
                        //tak copasno codingku
                        //cocok? cocok pol
                        //kodinganmu sg ndi wi yo sg iso nampilno?
                        //legend2 e sng jelas, terus ono setting nang pieChart episan
                        ...
                        l.setYEntrySpace(2f);
                        l.setYOffset(2f);
                        //iku sing Legend e nduwur iku?
                        //oh iku saiki set color e iso langsung gausah di lebokno array neh?
                        //iyo asline, gaperlu bolak balik dilebokno array, kakean alokasi gawe variable yo kurang apik
                        //okee2, sip suwun ya syadd
                        //yoi, tak close iki


                        //hmm iki emang butuh seakeh iki a yo? iyaa
                        //gawe opo description e? default e tulisan opo ngno lali aku trus tak ganti
                        //tak hapus ae yo? yawis hpus sek ae
//                            Description description = new Description();
//                            description.setText("Lead Indicator");
...
//                            final String[] month = new String[]{"Jan", "Feb", "Mar", "Apr", "May", "Jun"};
    ...
    ////            data.add(new ValueDataEntry(status[i], value_status[i]));
    ////        }
    ////
    ////        pie.data(data);
    ////
    ////        pie.title("Sales Lead Register");
    ////
    ...
    //        PieChart pieChart = findViewById(R.id.pieChart);
    //        pieChart.setUsePercentValues(true);
    //        ArrayList<PieEntry> xVals = new ArrayList<>();
    //
    //        for (int i = 0; i < values_status.toArray().length; i++){
    //
    //            xVals.add(new PieEntry(values_status.get(i), status.get(i)));
    //        }
    //
    //        PieDataSet dataSet = new PieDataSet(xVals,"");
    //        PieData  data = new PieData(dataSet );
